...
namespace UnityEngine.UI.Extensions
{
    ...
    public class RadialSlider : MonoBehaviour, IPointerEnterHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        {
            float angle = m_targetAngle * 360f;
            float percentage = angle / 360f;
            float displayValue = percentage * 100f;
            string displayText = Mathf.RoundToInt(displayValue).ToString();
            _onValueChanged.Invoke(displayValue);
            _onTextValueChanged.Invoke(displayText + "%");
        }
